...
class ConcatenatedCNN2LSTM(nn.Module):
    def __init__(self, spa_length, spa_width, ang_length, ang_width, feature_size, img_model, device, hidden_size=64, num_layers=1, sequence_length=100):
        super(ConcatenatedCNN2LSTM, self).__init__()
        self.num_layers = num_layers
        self.hidden_size  = hidden_size
        self.device = device
        ...
        self.spa_cnn = img_model(spa_width, spa_length, feature_size)
        self.ang_cnn = img_model(ang_width, ang_length, feature_size)
        self.lstm = nn.LSTM(2 * feature_size, hidden_size, batch_first=True)
        self.fc1 = nn.Sequential(
            nn.Linear(hidden_size, hidden_size//2),
            nn.ReLU(),
            nn.Linear(hidden_size//2, 1))
    ...
    def forward(self, spa, ang):
        ...
        out,_ = self.lstm(features, (h0, c0))      # out: (BS, L, H_out)
        out = out[:, -1]
        out = self.fc1(out)
        return out